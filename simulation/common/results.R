...
flatten_method_result <- function(result, method, cfg, replicate, data_seed,
  ...
  if (!is.null(error) || is.null(result)) {
    return(data.frame(
      ...
      permutation_scheme = as.character(.config_scalar(cfg, "permutation_scheme", "iid")),
      block_length = as.integer(.config_scalar(cfg, "block_length", NA_integer_)),
  ...
  data.frame(
    ...
    grid_size = as.integer(.config_scalar(cfg, "grid_size", NA_integer_)),
    object_size = as.integer(.config_scalar(cfg, "object_size", NA_integer_)),
    num_permut = as.integer(.result_scalar(result, "num_permut", as.integer(cfg$num_permut))),
...
summarize_tidy_results <- function(detail) {
  ...
  rows <- lapply(pieces, function(x) {
    ...
    data.frame(
      ...
      permutation_scheme = x$permutation_scheme[1L], block_length = x$block_length[1L],
